...
start() {
  ...
  *)
  ...
  if [ "$use_mod" -eq 1 ]; then
    ...
    if [ -f "$modpath" ]; then
      ret=$(insmod "$modpath" "$modargs")
      if [ "$ret" -eq 0 ]; then
        echo 'zram module loaded'
      else
        echo "module loading failed and exiting($ret)"
        exit $ret
      fi
    else
  ...
  while [ "$zramdev_num" -lt "$nr_zramdev" ]; do
    ...
    mkswap /dev/block/zram${zramdev_num} && (echo "mkswap ${zramdev_num}") || (
      echo "mkswap ${zramdev_num} failed and exiting(${?})"
      exit $?
    )
    swapon -p $swap_prio /dev/block/zram0 && (echo "swapon ${zramdev_num}") || (
      echo "swapon ${zramdev_num} failed and exiting(${?})"
      exit $?
    )
    ((zramdev_num++))
    ((swap_prio++))
...
stop() {
  ...
  if [ $swaps ]; then
    ...
    if [ $(lsmod | grep -c zram) -ne "0" ]; then
      rmmod zram && (echo "zram unloaded") || (
        echo "zram unload fail(${?})"
        exit $?
      )
    fi
  fi
